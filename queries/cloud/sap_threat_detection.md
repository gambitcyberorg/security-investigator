...
**Purpose:** Surface all SAP-related security alerts generated by the Sentinel SAP solution, with severity and frequency analysis.

**Use Case:** Investigation starting point — what SAP threats has Sentinel already detected? Use this to identify the most active alert types and prioritize further hunting.

<!-- cd-metadata
cd_ready: false
adaptation_notes: "SecurityAlert aggregation — not adapted for CD. Use for investigation context."
-->
```kql
// SAP Security Alerts — Correlation Summary
let Lookback = 30d;
SecurityAlert
| where TimeGenerated > ago(Lookback)
...
| summarize 
    ...
    FirstSeen = min(TimeGenerated),
    LastSeen = max(TimeGenerated),
    UniqueAlertIds = dcount(SystemAlertId)
    by AlertName
| order by AlertCount desc
| extend AlertFrequency = case(
    AlertCount >= 20, "🔴 Frequent — investigate pattern",
    AlertCount >= 5, "🟠 Recurring — review scope",
    "🟡 Rare — investigate individually")
```
